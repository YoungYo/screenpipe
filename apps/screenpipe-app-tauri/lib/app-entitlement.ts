...
export function isDevBillingBypassEnabled() {
  // ---- LOCAL UNLOCK -------------------------------------------------------
  // 本地自建版: 全部功能免登录可用。此函数是前端门禁的单一真相源 ——
  // hasAppEntitlement() 首行读它, AppEntitlementGate 的 shouldGate 也读它。
  // 恒为 true -> 付费墙消失、不再 stopScreenpipe、不再要求登录/订阅。
  // 保留 E2E_FORCE_BILLING_GATE_KEY 引用避免未使用告警。
  void E2E_FORCE_BILLING_GATE_KEY;
  return true;
}
